...
export function useLicenseState() {
    const context = useContext(LicenseStateContext);
    if(!context) {
        throw new Error("Cannot find LicenseProvider");
    }
    return context;
}

export function useLicenseDispatch() {
    const context =  useContext(LicenseDispatchContext);
    if(!context) {
        throw new Error("Cannot find LicenseProvider");
    }
    return context;
...
export function useLicenseNextId() {
    const context =  useContext(LicenseNextIdContext);
    if(!context) {
        throw new Error("Cannot find LicenseProvider");
    }
    return context;
